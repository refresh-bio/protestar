...
public:
	...
	// *******************************************************************************************
	int register_stream_no_lock(const std::string& stream_name)
	{
		return register_stream_impl(stream_name);
	}
	...
	// *******************************************************************************************
	bool add_part_no_lock(const int stream_id, const std::vector<uint8_t>& v_data, const uint64_t metadata = 0)
	{
		return add_part_impl(stream_id, v_data.data(), v_data.size(), metadata);
	}
