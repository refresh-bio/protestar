...
class ExtDictPdb : public ExtDictBase
{
protected:
        ...
        for (int i = 1; i < (int)v_chem.size(); i += 3)
            for (int j = 1; j < (int)v_chem.size(); j += 3)
                if (i != j)
                {
                    ...
                    type_specific_dict.insert(type_specific_dict.end(), v_chem[i].begin(), v_chem[i].end());
                    type_specific_dict.emplace_back(0);
                    type_specific_dict.insert(type_specific_dict.end(), v_chem[j].begin(), v_chem[j].end());
                    type_specific_dict.emplace_back(0);
